...
public class ExcelUtil {
	...
	public static <T> void listToExcel(List<T> list,
		...
		try {
			...
			// 1.计算一共有多少个工作表
			int sheetNum = (int) Math.ceil(list.size()
	...
	public static <T> List<T> excelToList(InputStream in, String sheetName,
		...
		try {
			...
			// 如果有列名不存在，则抛出异常，提示错误
			if (!isExist) {
				throw new Exception("Excel中缺少必要的字段[" + errorFieldName
						+ "]，或字段名称有误");
			}
			...
			// bug #3417 zhangfenghuang 2016-03-23
			// 存在重复行时，略过重复行不导入，之后正确的数据需要可以继续导入 begin
			// // 判断是否有重复行
			...
			// nullCols++;
			// throw new
			// Exception("第"+(i+1)+"行"+"【"+uniqueFields[j]+"】存在重复，请检查");
			// }
			// }
			// }
			// bug #3417 zhangfenghuang 2016-03-23
			// 存在重复行时，略过重复行不导入，之后正确的数据需要可以继续导入 end
			// 将sheet转换为list
			for (int i = 1; i < realRows; i++) {
				// 新建要转换的对象
	 ...
	 *             异常
	 */
	public static void setFieldValueByName(String fieldName, Object fieldValue,
			Object o) throws Exception {
	...
	public static void setColumnAutoSize(WritableSheet ws, int extraWith) {
		// 获取本列的最宽单元格的宽度
		//add by zhangRb 20180810设置列宽最大宽度不超过60 + 额外宽度
		int maxWidth = 60;
	...
	public static <T> void fillSheet(WritableSheet sheet, List<T> list,
		...
		// 填充表头
		//add by zhangRb 20180810 导出功能优化，表头显示优化
        ...
        wcf_title.setBackground(jxl.format.Colour.BLUE_GREY); // 设置单元格的背景颜色
		for (int i = 0; i < cnFields.length; i++) {
			Label label = new Label(i, 0, cnFields[i]);
			label.setCellFormat(wcf_title);
			sheet.addCell(label);
		}
		//add by zhangRb 20180810 导出功能优化，冻结表头
		...
		for (int index = firstIndex; index <= lastIndex; index++) {
			...
			for (int i = 0; i < enFields.length; i++) {
				...
				//add by zhangRb 20180810 导出功能优化，日期时间格式化
				String fieldValue = "";
				if (objValue instanceof Date && objValue != null) {
					fieldValue = dFormat.format(objValue);
				}else {
	 ...
	 * 写入临时日志
	 * 
	 * @author mengwenqi on 2015年8月18日
	 */
	public static void writeLog(String path, String flieName, String str) {
		try {
